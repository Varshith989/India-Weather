...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/85 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800/80 px-4 lg:px-8 py-3.5 transition-colors shadow-xs">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2.5 shrink-0 cursor-pointer">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] uppercase tracking-wider font-extrabold px-1.5 py-0.5 rounded bg-gradient-to-r from-sky-500 to-indigo-500 text-white">
              PRO
            </span>
          </div>
        </div>

        {/* Search Bar with Indian PIN & City Support */}
        <div className="relative flex-1 max-w-lg" ref={dropdownRef}>
          <form onSubmit={handleSubmit} className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Search Indian city or 6-digit PIN (e.g. Hyderabad, 110001)..."
              className="w-full pl-10 pr-20 py-2 bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 transition-all shadow-inner"
            />
            {query && (
              <button
                type="button"
            ...
            {isSearching && (
              <Loader2 className="absolute right-3.5 w-4 h-4 text-sky-500 dark:text-sky-400 animate-spin" />
            )}
          </form>

          {/* Autocomplete Dropdown */}
          {showDropdown && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50 divide-y divide-slate-100 dark:divide-slate-800/60 max-h-72 overflow-y-auto">
              {suggestions.map((loc, i) => (
                <button
                  ...
                  className="w-full px-4 py-2.5 text-left flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/70 transition-colors text-sm"
                >
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">{loc.state}</span>
                </button>
              ))}
            </div>
          )}
