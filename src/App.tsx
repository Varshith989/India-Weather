...
import { AlertTriangle, X, Loader2, Heart, Plus, PhoneCall, Volume2, VolumeX, Check } from "lucide-react";
...
export function App() {
  ...
  const cleanCurrentCity = location.name.split(",")[0].split("(")[0].trim();

  const isCurrentSaved = favorites.some(
    (fav) =>
      fav.toLowerCase() === cleanCurrentCity.toLowerCase() ||
      location.name.toLowerCase().includes(fav.toLowerCase()) ||
      fav.toLowerCase().includes(cleanCurrentCity.toLowerCase())
  );

  const handleToggleFavorite = () => {
    const targetName = cleanCurrentCity || location.name;
    if (isCurrentSaved) {
      const next = favorites.filter(
        (fav) =>
          fav.toLowerCase() !== targetName.toLowerCase() &&
          !location.name.toLowerCase().includes(fav.toLowerCase()) &&
          !fav.toLowerCase().includes(targetName.toLowerCase())
      );
      setFavorites(next);
      localStorage.setItem("fav_cities", JSON.stringify(next));
    } else {
      const next = [...favorites, targetName];
      setFavorites(next);
      localStorage.setItem("fav_cities", JSON.stringify(next));
    }
  };
  ...
  const handleRemoveFavorite = (cityName: string) => {
    const next = favorites.filter((fav) => fav.toLowerCase() !== cityName.toLowerCase());
    setFavorites(next);
    localStorage.setItem("fav_cities", JSON.stringify(next));
  };
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none w-full md:w-auto">
            ...
            {favorites.map((fav) => {
              ...
              return (
                <div
                  key={fav}
                  className={`group flex items-center rounded-xl text-xs font-semibold shrink-0 transition-all ${
                    isActive
                ...
                >
                  <button
                    ...
                    className={`py-1 cursor-pointer transition-all ${favorites.length > 1 ? "pl-3 pr-1.5" : "px-3"}`}
                  >
                    {fav}
                  </button>
                  {favorites.length > 1 && (
                    <button
                      ...
                      className={`pr-2 pl-0.5 py-1 text-[11px] font-bold leading-none cursor-pointer transition-colors ${
                        isActive
            ...
            <button
              ...
              title={
                isCurrentSaved
                  ? `Remove ${cleanCurrentCity} from favorites`
                  : `Save ${cleanCurrentCity} to quick favorites`
              }
              className={`px-2.5 py-1 rounded-xl text-xs font-semibold flex items-center gap-1 shrink-0 transition-all cursor-pointer active:scale-95 ${
                isCurrentSaved
                  ? "bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/15 dark:hover:bg-rose-500/25 text-rose-600 dark:text-rose-400 border border-rose-300 dark:border-rose-500/40 shadow-xs"
                  : "border border-dashed border-slate-300 hover:border-sky-400 dark:border-slate-700 dark:hover:border-sky-500 text-slate-500 hover:text-sky-600 dark:text-slate-400 dark:hover:text-sky-400"
              }`}
            >
              {isCurrentSaved ? (
                <>
                  <Check className="w-3.5 h-3.5 text-rose-500 stroke-[2.5]" />
                  <span>Saved</span>
                </>
              ) : (
                <>
